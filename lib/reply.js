...
function sendWebStream (payload, res, reply) {
  safeWriteHead(reply, res.statusCode)
  ...
  eos(res, function (err) {
    if (err != null) {
      if (res.headersSent && !errorLogged) {
        errorLogged = true
        logStreamError(reply.log, err, res)
      }
    }
    sourceOpen = false
    // Cancel the reader when the response is destroyed
  ...
  function onReadError (error) {
    ...
    if (sourceOpen) {
      ...
      if (res.headersSent) {
        if (!errorLogged) {
          errorLogged = true
          logStreamError(reply.log, error, res)
        }
        res.destroy()
      } else {
        onErrorHook(reply, error)
      }
    }
    reader.cancel().catch(noop)
  }
