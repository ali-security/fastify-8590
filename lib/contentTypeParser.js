...
const { safeParse: safeParseContentType } = require('fast-content-type-parse')
...
ContentTypeParser.prototype.add = function (contentType, opts, parserFn) {
  ...
  if (contentTypeIsString && contentType === '*') {
    ...
    if (contentTypeIsString) {
      ...
      this.parserList.unshift(new ParserListItem(normalizedContentType))
...
ContentTypeParser.prototype.getParser = function (contentType) {
  ...
  // Parse the content type using the old parser for parameter matching
  const parsed = safeParseContentType(originalContentType)

  // eslint-disable-next-line no-var
  for (var i = 0; i !== this.parserList.length; ++i) {
    const parserListItem = this.parserList[i]
    if (compareContentType(parsed, parserListItem)) {
      parser = this.customParsers.get(parserListItem.name)
      this.cache.set(ct, parser)
      return parser
    }
  }
  ...
  for (var j = 0; j !== this.parserRegExpList.length; ++j) {
    ...
    const testString = parserRegExp.isEssence ? parsed.type : originalContentType
    if (parserRegExp.test(testString)) {
